#! /usr/bin/env Rscript
#
# Modified by Kevin Glocker in 2022
...
assign_seg_class <- function(segment, allophoneClasses = NULL) {
    ...
    is_ambiguous <- (is_null + is_consonant + is_vowel + is_tone) > 1
    # populate vector
    ...
    # Try resolving ambiguous segment classes with allophone information
    if (length(is_ambiguous) > 0) {
        segment_class[is_ambiguous] <- sapply(
            segment[is_ambiguous],
            function(current_segment) {
                allophone_class <- allophoneClasses$Allophones == current_segment
                if (any(allophone_class)) {
                    # Take segment type from the phoneme this phone is an allophone of
                    as.character(allophoneClasses$SegmentClass[allophone_class])
                }
                else {
                    # If ambiguity can't be resolved with allophone information, fall back to NA
                    NA
                }
            }
        )
