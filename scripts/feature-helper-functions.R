...
get_feat_matrix_from_codepoints <- function(codepoints, table=feats) {
    ...
    if (class(rows) != "integer") {
        valid_indices = as.logical(sapply(rows, length))
        missing_ix <- which(!valid_indices)
        # TODO: Fill with NA instead of removing row for missing code point?
        warning("missing features for codepoint(s) ", codepoints[missing_ix], " will be removed from the feature matrix")
        rows <- as.integer(rows[valid_indices])
    }
    table[rows,]
}
...
simplify_contour_feats <- function(feat_vec) {
    ...
    # feat_vec <- as.character(feat_vec)
...
simplify_contour_feat <- function(feat_val) {
    feat_val <- as.character(feat_val)
